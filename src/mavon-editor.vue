...
export default {
  ...
  mounted() {
    ...
    // 将help添加到末尾
    if (this.$refs.help) document.body.appendChild(this.$refs.help);
  ...
  beforeDestroy() {
    if (this.$refs.help) document.body.removeChild(this.$refs.help);
  },
  getMarkdownIt() {
  ...
  methods: {
    ...
    // 监听ctrl + s
    save(val, render) {
      if (this.toolbars.save) this.$emit("save", val, render);
    },
    // 导航栏切换
